...
class backup_restore{
    ...
    // Constructor
 	function __construct($dbhost,$database,$dbUser ,$dbPass ,$tables="*" ) {
        ...
        $this->host = $dbhost;
 		...
 		$this->user = $dbUser;
 		$this->pass = $dbPass ;
 		$this->file = "$common_path/backups/buffernow.com.phoenix";
 		$this->tables =$tables;
        $this->msg='';
         }
    ...
    public function backup($file=""){
        global $common_path;
        if ($file<>""){
            $this->file=buildPath($common_path,'backups',$file);
        }
  ...
  //Ohh Wait ,When we create backup. lets save this time in log file
    $log=buildPath($common_path,"errorLogs","backup_log.log");
  ...
  //yeah its all done
    return "Restore Point Created Sucsessfully!!";
}
